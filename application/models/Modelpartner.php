...
class ModelPartner extends CI_Model
{
    ...
    public function list_partner_booking($data)
    {
        ...
        if ($data['booking_id']!='') {
            $query = $this->db->query("select bt.booking_id, mst.service_type_desc, mat.action_type_desc, mpm.pymt_methode_desc, bt.created_date, ar.full_name, ar.gender, ar.mobile_no, concat('".FULL_UPLOAD_PATH_PROFILE."', ua.profile_photo) profile_photo, bt.promo_code, bt.price_amount, bt.partner_profit_share, bt.latitude_request, bt.longitude_request, bt.service_rating, bt.user_comment, bt.cancel_by, mr.diagnosa from booking_trx bt inner join user_account ua on bt.user_id=ua.user_id left join account_relation ar on bt.relation_id=ar.relation_id left join mst_service_type mst on bt.service_type_id=mst.service_type_id left join mst_action_type mat on bt.action_type_id=mat.action_type_id left join mst_payment_methode mpm on bt.pymt_methode_id=mpm.pymt_methode_id left join medical_record mr on bt.booking_id=mr.booking_id where bt.partner_selected='".$data['user_id']."' and bt.booking_id=".$data['booking_id']."");
            return $query->result();
            ...
            $query = $this->db->query("select bt.booking_id, mst.service_type_desc, mat.action_type_desc, mpm.pymt_methode_desc, bt.created_date, ar.full_name, ar.gender, ar.mobile_no, concat('".FULL_UPLOAD_PATH_PROFILE."', ua.profile_photo) profile_photo, bt.promo_code, bt.price_amount, bt.partner_profit_share, bt.latitude_request, bt.longitude_request, bt.service_rating, bt.user_comment, bt.cancel_by, mr.diagnosa from booking_trx bt inner join user_account ua on bt.user_id=ua.user_id left join account_relation ar on bt.relation_id=ar.relation_id left join mst_service_type mst on bt.service_type_id=mst.service_type_id left join mst_action_type mat on bt.action_type_id=mat.action_type_id left join mst_payment_methode mpm on bt.pymt_methode_id=mpm.pymt_methode_id left join medical_record mr on bt.booking_id=mr.booking_id where bt.partner_selected='".$data['user_id']."'");
            return $query->result();
        }
    }
