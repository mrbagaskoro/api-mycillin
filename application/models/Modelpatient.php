...
class ModelPatient extends CI_Model {
  ...
  public function add_request($data) {
    ...
    $insert['promo_code'] = $promo_code = $data['promo_code'];
  ...
  {
      $query = $this->db->query("select pr.user_id, bt.created_date as order_date, bt.booking_id, pm.pymt_methode_desc, bt.service_type_id, st.service_type_desc, bt.partner_selected, pr.full_name as partner_name, pa.mobile_no, concat('".FULL_UPLOAD_PATH_PROFILE."', pa.profile_photo) profile_photo, mpt.partner_type_desc, ss.spesialisasi_desc, pr.no_SIP, pr.no_STR, pr.wilayah_kerja, bt.promo_code, bt.price_amount, bt.booking_status_id, bt.cancel_status from booking_trx bt inner join partner_account pa on bt.partner_selected=pa.user_id left join mst_service_type st on bt.service_type_id=st.service_type_id left join mst_payment_methode pm on bt.pymt_methode_id=pm.pymt_methode_id left join partner_profile pr on pa.user_id=pr.user_id left join mst_partner_type mpt on pr.partner_type_id=mpt.partner_type_id left join mst_spesialisasi ss on pr.spesialisasi_id=ss.spesialisasi_id where bt.user_id='$user_id' and bt.cancel_status='N' and bt.booking_status_id != '04' ");
      return $query->result();
  }

  public function list_history_completed($user_id)
  {
      $query = $this->db->query("select bt.created_date as order_date, bt.booking_id, bt.service_type_id, st.service_type_desc, bt.partner_selected, pr.full_name as partner_name, pr.partner_type_id, pr.spesialisasi_id, concat('".FULL_UPLOAD_PATH_PROFILE."', pa.profile_photo) profile_photo, pa.mobile_no, bt.pymt_methode_id, mpm.pymt_methode_desc, bt.promo_code, bt.service_rating, bt.price_amount, bt.cancel_by, cr.cancel_reason_desc as cancel_reason_by_user, crp.cancel_reason_desc as cancel_reason_by_partner, mr.diagnosa, mat.action_type_desc, mr.prescription_type_id, bt.booking_status_id, bt.cancel_status from booking_trx bt inner join partner_profile pr on bt.partner_selected=pr.user_id left join mst_service_type st on bt.service_type_id=st.service_type_id left join partner_account pa on bt.partner_selected=pa.user_id left join mst_payment_methode mpm on bt.pymt_methode_id=mpm.pymt_methode_id left join mst_cancel_reason cr on bt.cancel_reason_id=cr.cancel_reason_id left join mst_cancel_reason_partner crp on bt.cancel_reason_id=crp.cancel_reason_id left join medical_record mr on bt.booking_id=mr.booking_id left join mst_action_type mat on bt.action_type_id=mat.action_type_id where (bt.user_id='$user_id' and bt.booking_status_id='04' and bt.cancel_status = 'N') or (bt.user_id='$user_id' and bt.booking_status_id!= '04' and bt.cancel_status='Y') ");
      return $query->result();
  }
}