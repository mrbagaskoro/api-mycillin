...
class Medicalhost extends Controlapidoc{
  ...
  public function get_loc_doc_get(){
    //$this->validate_jwt();

    $user_data = $this->ma->get_loc_doc();
  ...
  public function update_loc_doc_post(){
    $this->validate_jwt();
  ...
  public function validate_account_token_get(){
    $email = $this->input->get('email', TRUE);
    $token_id = $this->input->get('activation_code', TRUE);

    $user_data = $this->ma->is_valid_user($email);
    if ($user_data->user_status == '2') {
      if ($this->ma->is_valid_token($email,$token_id) != NULL) {
        if ($this->ma->change_user_state($email)) {
            $this->email->from(EMAIL_ADDR, 'Lucy@MyCillin', EMAIL_ADDR);
          $this->email->to($user_data->user_email_address);
                                                  ...
                                                  <table cellpadding="0" cellspacing="0" width="100%" bgcolor="#ffffff">
                                                    ...
                                                    <tr>
                                                      <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
                                                        <b>Hi '.$user_data->user_full_name.',</b><br>
                                                    ...
                                                    <tr>
                                                      <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
                                                        ...
                                                        Account name : '.$user_data->user_full_name.'<br>
                                                        Email : '.$user_data->user_email_address.'<br>
                                                      </td>
                                                    </tr>
                                                    <tr>
                                                      <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
  ...
  function register_guess_host_post(){
    $this->form_validation->set_rules('email','EMAIL','trim|max_length[50]|required');
    $this->form_validation->set_rules('name','NAMA LENGKAP','trim|max_length[30]|required');
    ...
    if ($this->form_validation->run()==FALSE) {
      ...
      $data = [
        'user_email_address'=>$this->post('email',TRUE),
        'user_full_name'=>$this->post('name',TRUE),
        'user_password'=>$this->encrypt->encode($this->post('password')),
        'user_create'=>$this->post('email',TRUE),
        'date_create'=>date("Y-m-d H:i:s"),
        'user_status'=>'2'
      ];
      //jika berhasil di masukan maka akan di respon kembali sesuai dengan data yang di masukan
      $user_exist = $this->ma->is_valid_num_user($data['user_email_address']);
      if ($user_exist > 0) {
        $this->bad_req('email already registered');
      } else {
        if ($result = $this->ma->register_guess_host($data)) {

          $this->email->from(EMAIL_ADDR, 'Lucy@MyCillin', EMAIL_ADDR);
          $this->email->to($data['user_email_address']);
                                                  ...
                                                  <table cellpadding="0" cellspacing="0" width="100%" bgcolor="#ffffff">
                                                            ...
                                                            <td style="width:200px;background:#008000;">
                                                              <div><!--[if mso]>
                                                                <v:rect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="#" style="height:40px;v-text-anchor:middle;width:200px;" stroke="f" fillcolor="#008000">
                                                                  ...
                                                                  <center>
                                                                <![endif]-->
                                                                    <a href="'.base_url().'api/activation?email='.$data['user_email_address'].'&activation_code='.$result->token_id.'"
                                                    ...
                                                    <tr>
                                                      <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
                                                        ...
                                                        <p>'.base_url().'api/activation?email='.$data['user_email_address'].'&activation_code='.$result->token_id.'</p>
                                                      </td>
                                                    </tr>
                                                    <tr>
                                                      <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
                                                    ...
                                                    <tr>
                                                      <td style="padding-top:20px;background-color:#ffffff;">
                                                        <p style="text-align: justify; color: grey; padding-left: 10px">This message was sent to '.$data['user_email_address'].'.
  ...
  $user_data = $this->ma->is_valid_user($data['email']);
	...
	if ($user_data) {
		if ($user_data->user_status == '1') {
			if ($update = $this->ma->forgot_password($data['email'])){
				$this->email->from(EMAIL_ADDR, 'Lucy@MyCillin', EMAIL_ADDR);
			  $this->email->to($user_data->user_email_address);
													  ...
													  <table cellpadding="0" cellspacing="0" width="100%" bgcolor="#ffffff">
														...
														<tr>
														  <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
															...
															Account name : '.$user_data->user_full_name.'<br>
															Email : '.$user_data->user_email_address.'<br>
															<b>Your New Password : '.$update.'</b><br>
														  </td>
														</tr>
														<tr>
														  <td valign="top" style="padding-bottom:20px; background-color:#ffffff;">
